...
class Regular(Frame):
    def __init__(self, winmain, current):
        ...
        topframe = Frame(self)
        topframe.pack()
        switch = Button(topframe, text='Scientific', command=lambda: current.show_calc(Scientific,'Scientific'))
        switch.pack(pady=7, side=LEFT)
        switch.configure(background='blue', foreground='white')

        file_btn = Button(topframe, text='History', command=current.history)
        file_btn.pack(pady=7, side=LEFT)
        file_btn.configure(background='green', foreground='white')
        ...
        for btn in self.btns:
            ...
            if btn == 'AC':
                butn = Button(btnframe, text=btn, height=2, width=4, command=self.clear)
                butn.grid(row=row, column=column, pady=2, padx=3)
                butn.configure(background="yellow")
                column += 1

            elif btn == '<-':
                butn = Button(btnframe, text=btn, height=2, width=4, command=self.delete)
                butn.grid(row=row, column=column, pady=2, padx=3)
                butn.configure(background="red")
                column += 1

            elif btn == '=':
                butn = Button(btnframe, text=btn, height=2, width=4, command=self.equal)
                butn.grid(row=row, column=column, pady=2, padx=3)
                butn.configure(background="yellow")
                column += 1

            else:
                butn = Button(btnframe, text=btn, height=2, width=4,
                              command=lambda char=btn: self.addch(char))
                butn.grid(row=row, column=column, pady=2, padx=3)
                butn.configure(background="cyan")
                column += 1
...
class Scientific(Frame):
    def __init__(self, winmain, current):
        ...
        switch = Button(topframe, text='Regular', command=lambda: current.show_calc(Regular,'Regular'))
        switch.pack(pady=7, side=LEFT)
        switch.configure(background='blue', foreground='white')
        ...
        self.angle_btn = Button(topframe, text='Radians', command=self.angle_button)
        self.angle_btn.pack(pady=7, side=LEFT)
        self.angle_btn.configure(background='brown', foreground='white')

        self.mode_btn = Button(topframe, text='Sci. Notation', command=self.mode)
        self.mode_btn.pack(pady=7, side=LEFT)
        ...
        for btn in self.btns:
            ...
            if btn == 'AC':
                butn = Button(btnframe, text=btn, height=2, width=4, command=self.clear)
                butn.grid(row=row, column=column, pady=1, padx=2)
                butn.configure(background="yellow")
                column += 1
                ...
                butn = Button(btnframe, text=btn, height=2, width=4,
                              command=lambda char=btn: self.addch(char))
                butn.grid(row=row, column=column, pady=1, padx=2)
                butn.configure(background="orange")
                column += 1
                ...
                butn = Button(btnframe, text=btn, height=2, width=4,
                              command=lambda char=btn: self.addch(char))
                butn.grid(row=row, column=column, pady=1, padx=2)
                butn.configure(background="sky blue")
                column += 1
